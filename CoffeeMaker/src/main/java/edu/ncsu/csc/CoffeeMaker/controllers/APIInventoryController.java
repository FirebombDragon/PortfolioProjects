...
public class APIInventoryController extends APIController {
    ...
    public ResponseEntity updateInventory ( @RequestBody final Inventory inventory ) {
        ...
        for ( final Ingredient i : inventory.getIngredients() ) {
            if ( inventoryCurrent.getAmountByType( i.getIngredient() ) == 0 ) {
                inventoryCurrent.addCustomIngredient( i );
            }
            else {
                inventoryCurrent.changeAmountByType( i.getIngredient(), i.getAmount() );
            }
        }
        inventoryCurrent.addIngredients( inventory.getAmountByType( "Coffee" ), inventory.getAmountByType( "Milk" ),
                inventory.getAmountByType( "Sugar" ), inventory.getAmountByType( "Chocolate" ) );
