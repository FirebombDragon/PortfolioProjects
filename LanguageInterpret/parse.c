/**
   ...
   This file holds all the functions necessary to parse and execute arguments 
...
/**
   This function parses expressions from the given file
   @param tok Token being parsed
   @param fp File pointer for use
   @return an expression based on the token and input
*/
...
/** Initial capacity for resizable compound statement array */
...
/** Count of the line being parsed */
...
static Expr *parseTerm( char *tok, FILE *fp )
{
  ...
  if ( tok[ 0 ] == '-' || isdigit( tok[ 0 ] ) ) {
    ...
    if (tok[1] == '\n') {
      char *newLine = (char *)malloc(NEW_LINE_LENGTH);
      newLine[0] = '\n';
      newLine[1] = '\0';
      Expr *exp = makeLiteral(makeString(newLine));
      free(newLine);
      return exp;
    }
    else {
      tok = tok + 1;
      char *newToken = (char *)malloc(strlen(tok));
      int stringLen = strlen(tok);
      if (tok[strlen(tok) - 1] == '\"') {
        stringLen = strlen(tok) - 1;
      }
      strncpy(newToken, tok, stringLen);
      newToken[strlen(tok) - 1] = '\0';
      Expr *exp = makeLiteral(makeString(newToken));
      free(newToken);
      return exp;
    }
